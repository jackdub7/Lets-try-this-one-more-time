...
export default function TimesScreen() {
  ...
  const saveTimeEntries = async (entries: TimeEntry[]) => {
    try {
  ...
  const handleAddTime = () => {
    if (!newEntry.swimmerName || !newEntry.distance || !newEntry.time || !newEntry.testSet) {
    ...
    const entry: TimeEntry = {
      ...
      swimmerName: newEntry.swimmerName,
      stroke: newEntry.stroke,
      distance: parseInt(newEntry.distance),
      time: formattedTime,
      date: new Date().toISOString().split('T')[0],
      testSet: newEntry.testSet,
      notes: newEntry.notes,
    };

    const updatedEntries = [entry, ...timeEntries];
  ...
  const handleBulkImport = () => {
    ...
    try {
      ...
      lines.forEach((line, index) => {
        ...
        if (parts.length >= 5) {
          const [swimmerName, stroke, distance, time, testSet, notes = ''] = parts;
          
          // Basic validation
          if (swimmerName && stroke && distance && time && testSet && validateTimeInput(time)) {
      ...
      if (newEntries.length > 0) {
        const updatedEntries = [...newEntries, ...timeEntries];
        setTimeEntries(updatedEntries);
        saveTimeEntries(updatedEntries);
        setBulkData('');
        setShowBulkModal(false);
        Alert.alert('Success', `${newEntries.length} time entries imported successfully!`);
      } else {
          ...
          {recentTimes.map((entry) => (
            <View key={entry.id} style={styles.timeCard}>
              <View style={styles.timeHeader}>
                <Text style={styles.swimmerName}>{entry.swimmerName}</Text>
              ...
              <View style={styles.timeDetails}>
                ...
                <Text style={styles.timeDetail}>{entry.testSet}</Text>
              </View>
        ...
        <SafeAreaView style={styles.modalContainer}>
          ...
          <ScrollView style={styles.modalContent}>
            <View style={styles.inputGroup}>
              <Text style={styles.inputLabel}>Swimmer Name *</Text>
              <TextInput
                style={styles.textInput}
                value={newEntry.swimmerName}
                onChangeText={(text) => setNewEntry({ ...newEntry, swimmerName: text })}
        ...
        <SafeAreaView style={styles.modalContainer}>
          ...
          <View style={styles.modalContent}>
            ...
            <Text style={styles.bulkExample}>
              Format: Swimmer, Stroke, Distance, Time, Test Set
            </Text>
            ...
            <TextInput
              ...
              multiline
              numberOfLines={10}
              placeholder="John Doe, Freestyle, 100, 1:02.45, Time Trial&#10;Jane Smith, Backstroke, 200, 24.5, Sprint Set"
              textAlignVertical="top"
            />
          </View>
        </SafeAreaView>
